...
const createOrder = async (user, club) => {
  ...
  try {
    ...
    const paymentData = {
      ...
      currency: 'BDT',
      tran_id: tran_id,
      success_url: `http://localhost:${config.port}/v1/sslcommerz/success/${tran_id}`,
      fail_url: `http://localhost:${config.port}/v1/sslcommerz/failed/${tran_id}`,
      cancel_url: `http://localhost:${config.port}/v1/sslcommerz/cancel/${tran_id}`,
      shipping_method: 'NO',
